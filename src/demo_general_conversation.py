...
def schedule_chitchat(session, pool, theme=None, topics_of_interest=None, all_dialogs=None, completed_ids=None):
    """
    Try to append one runnable chitchat to the current session.
...
# def pick_chitchat(pool, theme=None, topics_of_interest=None):
#     """
#     Pick one chitchat from pool. If topics_of_interest is provided, prefer dialogs
#     whose topics intersect interests. Returns a dialog or None.
#     """
#     candidates = [d for d in pool if isinstance(d, ChitchatDialog) and (theme is None or d.theme == theme)]
#     if not candidates:
#         return None
#     if topics_of_interest:
#         matched = [d for d in candidates if topic_match(d, topics_of_interest)]
#         if matched:
#             candidates = matched
...
def select_session_block(mini_dialogs, thread=None, theme=None, topics_of_interest=None, completed_ids=None):
    ...
    # c1 = pick_chitchat(pool, theme=theme, topics_of_interest=topics_of_interest)
    # if c1:
    #     session.append(c1)
    #     pool.remove(c1) 
    ...
    # # NEW: prefer any interest-matching chitchat first, then fall back to themed pick
    # c2 = None
    # if topics_of_interest:
    #     interest_cands = [d for d in pool if isinstance(d, ChitchatDialog) and topic_match(d, topics_of_interest)]
    #     if interest_cands:
    #         if theme:
    #             same_theme = [d for d in interest_cands if d.theme == theme]
    #             c2 = random.choice(same_theme or interest_cands)
    #         else:
    #             c2 = random.choice(interest_cands)
    # if not c2:
    #     c2 = pick_chitchat(pool, theme=theme, topics_of_interest=topics_of_interest)
    # if c2:
    #     session.append(c2)
    #     pool.remove(c2)
...
#     # Class to handle loading and saving of all sessions history
#     def __init__(self):
#         self.sessions = []
    
#     def add_session(self, session_history):
#         self.sessions.append(session_history)
...
if __name__ == '__main__':
    ...
    # # Optional: force a fresh run ignoring previously completed dialogs
    # if os.environ.get("FORCE_FRESH_SESSION") == "1":
    #     print("[DEBUG] FORCE_FRESH_SESSION=1 -> ignoring previously completed dialogs for this run")
    #     completed_dialogs = set()
    # # Debug: show continuity state
    # try:
    #     print("[DEBUG] Completed dialogs loaded:", sorted(list(completed_dialogs)))
    #     print("[DEBUG] Topics of interest loaded:", topics_of_interest)
    # except Exception:
    #     pass
...
# # NEW
#     session_block = select_session_block(mini_dialogs, thread="dreams", theme="nature", topics_of_interest=topics_of_interest)
