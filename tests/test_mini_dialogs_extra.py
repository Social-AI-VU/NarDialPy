...
def test_extract_open_value_quotes_and_tokens():
    ...
    assert Dialog.extract_open_value("I love 'apples' so much") == 'apples'
    assert Dialog.extract_open_value('She said "banana" is nice') == 'banana'

    # Last alphabetic token should be returned when no quotes
    assert Dialog.extract_open_value('My favorite animal is a zebra') == 'zebra'

    # If no alphabetic tokens, return trimmed original
    assert Dialog.extract_open_value('  12345  ') == '12345'
